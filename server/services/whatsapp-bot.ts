  ...
  private async setupClient() {
    try {
      ...
      this.client.on("qr", async (qr: string) => {
        try {
          ...
          await storage.addBotLog({
            level: "info",
            message: "QR Code generated. Ready for scanning.",
          });
        } catch (err) {
      ...
      this.client.on("ready", async () => {
        ...
        await storage.addBotLog({
          level: "info",
          message: `🤖 Bot ID: ${this.client.info.wid._serialized}`,
        });
      });
      ...
      await storage.addBotLog({
        level: "error",
        message: "Failed to setup WhatsApp client",
      });
    }
  }
  ...
  private async handleMessage(msg: any) {
    try {
      ...
      if (isTagged) {
        if (
          ...
          Date.now() - this.conversations[chatId].last > this.CONTEXT_WINDOW
        ) {
          this.conversations[chatId] = { last: Date.now(), history: [] };
        }
        this.conversations[chatId].last = Date.now();
        this.conversations[chatId].history.push({
          role: "user",
          content: msg.body,
        });
        ...
        const aiReply = await this.generateAIReply(
          msg.body,
          this.conversations[chatId].history,
        ...
        await storage.addBotLog({
          level: "info",
          message: "🤖 AI response generated for user mention",
        });
        return;
      }

      const isPromo =
        this.isPromotionalText(msg.body) ||
        (msg.hasMedia && (await this.isPromotionalImage(msg)));
      if (isPromo && config.forwardToGroupId) {
        const targetChat = await this.client.getChatById(
          config.forwardToGroupId,
        );
    ...
    try {
      ...
      console.error("❌ Error sending forwarded confirmation:", err);
        ...
        await storage.addBotLog({
          level: "warn",
          message: "🔍 Promotional message detected and forwarded",
        });
      }
    } catch (err) {
